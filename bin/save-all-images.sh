...
NGINX_INGRESS_CONTROLLER_VERSION=0.9.0

GCR_URL=gcr.io/google_containers
CALICO_URL=quay.io/calico
COREOS_URL=quay.io/coreos
INGRESS_URL=quay.io/kubernetes-ingress-controller
...
ingress_images=(nginx-ingress-controller:${NGINX_INGRESS_CONTROLLER_VERSION})
...
for image in ${ingress_images[@]} ; do
    echo "begin to save ${image} ..."
    name=$(echo ${image} | awk -F":" '{print $1}')
    docker save -o ${SAVE_DIR}/${name}.tar ${INGRESS_URL}/${image}
    echo "save ${image} finished"
done
